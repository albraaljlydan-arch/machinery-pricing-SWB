<script lang="ts">
  ...
  async function load() {
    ...
    if (!userId) return;
    const { data, error } = await supabase.from('customer_requests').select('*').eq('assigned_designer_id', userId).order('created_at', { ascending: false });
      ...
      <table>
        ...
        <tbody>
          {#each requests as r (r.id)}
            <tr class="clickable-row" on:click={() => goto(`/designer/requests/${r.id}`)}>
