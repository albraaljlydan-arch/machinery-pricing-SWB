<script lang="ts">
  ...
  async function load() {
    ...
    const ids = [...new Set(requests.flatMap((r) => [r.customer_id, r.assigned_designer_id]).filter((id): id is string => !!id))];
    if (ids.length > 0) {
      ...
      <table>
        <thead>
          <tr>
            ...
            <th>{t($locale, 'assignDesignerLabel')}</th>
        ...
        <tbody>
          {#each requests as r (r.id)}
            <tr class="clickable-row" on:click={() => goto(`/admin/requests/${r.id}`)}>
              <td class="name">{r.title}</td>
              <td>{names[r.customer_id] ?? '—'}</td>
              <td>{r.spec_data?.machineType ?? '—'}</td>
              <td>{r.assigned_designer_id ? (names[r.assigned_designer_id] ?? '—') : t($locale, 'notAssignedYet')}</td>
